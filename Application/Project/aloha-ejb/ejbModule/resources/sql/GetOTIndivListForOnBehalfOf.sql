...
FROM
    ...
    INNER JOIN ALOHA.OT_HEADER HDR                           
        ON HDR.HEADER_ID = DTL.HEADER_ID
 		AND HDR.EMPL_USER_ID <> DTL.SUBM_USER_ID
...
WHERE
   DTL.SUBM_USER_ID = ?
AND
    DTL.DETAIL_SEQ = (SELECT MAX(INNER_DTL.DETAIL_SEQ) FROM ALOHA.OT_DETAIL INNER_DTL WHERE INNER_DTL.HEADER_ID = DTL.HEADER_ID)
ORDER BY
    PP.START_DATE DESC