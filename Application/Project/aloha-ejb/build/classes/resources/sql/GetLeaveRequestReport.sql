SELECT FacilityCd, FacilityDesc, AreaCd, AreaDesc, TeamCd, TeamDesc, eeid, last_name, first_name, middle_name,  YYYY, PP, leavedt, leavetype,  sum(ETAMS_LeaveHrs) leaveHrsEtams, sum(ALOHA_LeaveHrs) leaveHrsAloha
FROM
(    select team.etams_uid eeid, (dt.calendar_date - 14 + ead.dayx) LeaveDt, 
           ead.work_code || ' - ' || ead.work_code_desc LeaveType, 
           decode(substr(work_start, 4, 1), 'H', to_number(substr(work_start, 1, 3))/10, to_number(to_date(work_stop, 'hh24mi') - to_date(work_start, 'hh24mi'))*24) ETAMS_LeaveHrs, to_number(null) ALOHA_LeaveHrs,
    ...
    where (
            ( (ea.type = 'P') ) 
            OR
            ( (ea.type = 'A') and (trunc(ea.process_date) between trunc(dt.calendar_date - 13) and trunc(dt.calendar_date + 1)) ) 
            OR
            ( (ea.type = 'C') and (trunc(ea.process_date) between trunc(dt.calendar_date) and trunc(dt.calendar_date + 7)) )
          )
     and ea.ssn = team.etams_ssn 
...
UNION ALL
    select LRH.EMPLOYEE_USER_ID eeid, LRI.LEAVE_DATE leavedt, LRT.PRIMARY_CODE || ' - ' || LRT.PRIMARY_CODE_DESC leavetype,  
    to_number(null) ETAMS_LeaveHrs, LRI.LEAVE_HOURS ALOHA_LeaveHrs, 
...
GROUP BY eeid, first_name, last_name, middle_name, FacilityCd, AreaCd, TeamCd, FacilityDesc, AreaDesc, TeamDesc, YYYY, PP, leavedt, leavetype
