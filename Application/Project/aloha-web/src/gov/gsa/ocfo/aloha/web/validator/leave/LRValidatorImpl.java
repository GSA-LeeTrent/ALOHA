...
public abstract class LRValidatorImpl implements LRValidator, Serializable {
	private static final long serialVersionUID = -7054957806727589673L;
	...
	public LRValidatorImpl(PayPeriod payPeriod, List<LeaveItem> priorRequestItems) throws AlohaValidationException {
	...
	private void validateTotalHoursForPayPeriod(List<ScheduleItem> currentRequest) throws AlohaValidationException {
...
//		System.out.println("----------------------------------------------------");			
	...
	private void validateHoursForEachPayPeriodDay(List<ScheduleItem> currentRequest) throws AlohaValidationException {
		AlohaValidationException valException = new AlohaValidationException();
		// VALIDATE LEAVE HOURS FOR EACH PAY PERIOD DAY
		for ( ScheduleItem si : currentRequest ) {
			String siDateKey = DateUtil.formatDate(si.getCalendarDate(), DateUtil.DateFormats.YYYYMMDD);
			...
			BigDecimal newLeaveHoursForDay = si.getNumberOfLeaveHours();
...
//			System.out.println("----------------------------------------------------");			
			...
			if ( newLeaveHoursForDay != null) {
				...
				if ( newLeaveHoursForDay.compareTo(remainingLeaveHoursForDay) <= 0 ) {
					...
					String formattedLeaveDate = DateUtil.formatDate(si.getCalendarDate(), DateUtil.DateFormats.DEFAULT_FORMAT);
					StringBuilder errMsg = new StringBuilder();
	...
	private void validateLeaveTypesPerDay(List<ScheduleItem> currentRequest) throws AlohaValidationException {
		AlohaValidationException valException = new AlohaValidationException();
...
//			System.out.println("ppLeaveDay.getLeaveTypeCodes().containsKey(si.getSelectedLeaveTypeCode()): " + ppLeaveDay.getLeaveTypeCodes().containsKey(si.getSelectedLeaveTypeCode()));
			...
			String selectedLeaveTypeCode = si.getSelectedLeaveTypeCode();
	...
	private BigDecimal sumLeaveHoursForCurrentRequest(List<ScheduleItem> currentRequest ) {
		BigDecimal total = BigDecimal.ZERO;
		for ( ScheduleItem si : currentRequest) {
			if ( si.getNumberOfLeaveHours() != null) {
				total = total.add(si.getNumberOfLeaveHours());				
			}
		}
		return total;
	}
	...
	private void validateDateRangeForCurrentRequest(List<ScheduleItem> currentRequest) throws AlohaValidationException {
		AlohaValidationException valException = new AlohaValidationException();
		
		for ( ScheduleItem si :  currentRequest) {
			String siDateKey = DateUtil.formatDate(si.getCalendarDate(), DateUtil.DateFormats.YYYYMMDD);
			if ( ! this.ppLeaveDays.containsKey(siDateKey)) {
				StringBuilder errMsg = new StringBuilder();
				...
				errMsg.append(DateUtil.formatDate(si.getCalendarDate(), DateUtil.DateFormats.DEFAULT_FORMAT));
				errMsg.append("). ");				
				valException.addErrorMessage(errMsg.toString());
			}
		}
	...
	private void debug() {
		for (Map.Entry<String, PPLeaveDay> entry : this.ppLeaveDays.entrySet()) {
			PPLeaveDay ppLeaveDay = entry.getValue();
			System.out.println(ppLeaveDay.getKey() + " : " + ppLeaveDay.getDay());
		}
	}
	*/
}