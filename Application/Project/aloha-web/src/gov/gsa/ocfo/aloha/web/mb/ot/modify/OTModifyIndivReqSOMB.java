...
@ManagedBean(name=OTModifyIndivReqSOMB.MANAGED_BEAN_NAME)
@ViewScoped
public class OTModifyIndivReqSOMB extends OTModifyIndivReqAbstractMB {
	private static final long serialVersionUID = 1460791458362902191L;
	public static final String MANAGED_BEAN_NAME = "otModifyIndivReqSOMB";
	...
	protected void checkAccessRights() throws AuthorizationException {
		...
		if ( ! this.userMB.isSubmitOwn() ) {
			...
			String errMsg = ErrorMessages.getInstance().getMessage(ErrorMessages.OT_MODIFY_UNAUTHORIZED_SO, params);
		...
		if ( ! this.otDetail.isEmployeeUserId(this.userMB.getUserId()) ) {
			...
			String errMsg = ErrorMessages.getInstance().getMessage(ErrorMessages.OT_MODIFY_UNAUTHORIZED_SO_SPECIFIC, params);
			FacesContextUtil.getHttpSession().setAttribute(AlohaConstants.UNAUTHORIZED_MESSAGE, errMsg);
			throw new AuthorizationException(errMsg);
		}
	}
	...
	protected void checkIfLegalOperation() throws IllegalOperationException {
		if 	(  ! this.otDetail.isModifiableBySubmitOwn() ) {
			Object[] params = { this.otDetail.getRequestId(), this.otDetail.getStatus().getName() };
			String errMsg = ErrorMessages.getInstance().getMessage(ErrorMessages.OT_MODIFY_INVALID_STATUS_SO, params);
